...
def get_predictions_for_docking(feature_dir, scaler_path, model_dir, sample_id, out_dir):
    t0 = time.time()
    ...
    if not os.path.exists(npy_path):
        npy_path = os.path.join(feature_dir, f'{sample_id}_pocket.npy')
        if not os.path.exists(npy_path):
...
def main():
    ...
    parser.add_argument('--scaler_path', type=str, default='code_pkg/pretrain_data_standard_minmax_6channel_large.sav')
    ...
    parser.add_argument('--sample_id', type=str, required=True, help='id (e.g., pdbid)')
    parser.add_argument('-o', '--outdir', type=str, default='outputs', help='Path to output directory.')
    args = parser.parse_args()
    
    if args.function == 'docking':
        get_predictions_for_docking(args.feature_dir, args.scaler_path, args.model_dir, args.sample_id, args.outdir)
